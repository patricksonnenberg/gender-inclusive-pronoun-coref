...
class Coreferee():
    def get_corefs(self, sentence):
        """
        This method runs a sentence through the Coreferee to obtain the coreferences present in
        each sentence. A list of these predicted coreferences is returned.
        """
        doc = nlp(sentence)
        doc._.coref_chains.print()
        if len(doc._.coref_chains) > 0:
            result = doc._.coref_chains[0].pretty_representation
            split_item = re.split(':|\(|\)', result)  # Cleaning up the predictions to meet my format
...
if __name__ == "__main__":
    compare = Coreferee().compare_results('new_sentences small.tsv')